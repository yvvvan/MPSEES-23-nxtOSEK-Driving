...
int Mapping::exec_thread() {
    ...
    std::map<std::tuple<double, double> , std::array<int, 4>> throughtput_intersection;
    ...
    std::map<int , std::array<int, map_size>> cost_map; // costs between intersections
    ...
    */
    ...
    while (this->blackboard.mapping_enabled.get()) {
        // direction_last:   if at intersection( direction not unknow), pass the old value to .
        // direction:        initial direction with unknow
        ...
        if (station != station_t::STATION_UNKNOWN && (station_connection[station][0] == -1 || station_connection[station][1] == -1)){
            ...
            if (index_intersection.size() > 0){
                ...
                //station_connection[station][0] = index_intersection.find(intersection_last)->second;
                station_duration[station][0] = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - intersection_time).count();
            }
        ...
        if (intersection_detected && !intersection_detected_last) {
            ...
            if (!is_recorded_intersection) {
                ...
                throughtput_intersection.insert(std::pair<std::tuple<double, double>,std::array<int, 4>>(intersection_current,exits_visit_stat));
                // initial the connection and duration
                index_intersection_current = index_intersection.size();
                connection_map.insert(std::pair<int, std::array<int, 4>>(index_intersection_current, exits_visit_stat));
                cost_intersection.insert(std::pair<int, std::array<int, 4>>(index_intersection_current, exits_visit_stat));
                cost_map.insert(std::pair<int, std::array<int, map_size>>(index_intersection_current, std::array<int, map_size>{}));
                // index the new intersection
                index_intersection.insert(std::pair<std::tuple<double, double>, int>(intersection_current, index_intersection_current));
            }
            // if intersection already visited
            else {
                index_intersection_current = index_intersection.find(intersection_current)->second;
                // get the exits stat
                std::array<int, 4> exits_visit_stat = throughtput_intersection.find(intersection_current)->second;
                // comming direction +1
                ...
                throughtput_intersection.find(intersection_current)->second = exits_visit_stat;
            }

            // update the station connection(going direction)
            if (station_last != station_t::STATION_UNKNOWN && station_connection[station_last][1] == -1){
                station_connection[station_last][1] = index_intersection.find(intersection_current)->second;
            }

            // update the map
            if (index_intersection.size() > 1){
                //index_intersection_current = index_intersection.find(intersection_current)->second;
                //index_intersection_last = index_intersection.find(intersection_last)->second;
                
                // set connection
                std::array<int, 4> connection_current = connection_map.find(index_intersection_current)->second;
                std::array<int, 4> connection_last = connection_map.find(index_intersection_last)->second;
                connection_current.at(exit_current) = index_intersection_last;
                connection_last.at(exit_last) = index_intersection_current;

                // set duration
                std::array<int, 4> durations_current = cost_intersection.find(index_intersection_current)->second;
                std::array<int, 4> durations_last = cost_intersection.find(index_intersection_last)->second;
                ...
                connection_map.find(index_intersection_current)->second = connection_current;
                connection_map.find(index_intersection_last)->second = connection_last;
                cost_intersection.find(index_intersection_current)->second = durations_current;
                cost_intersection.find(index_intersection_last)->second = durations_last;

                // update the cost_map
                std::array<int, map_size> cost_current = cost_map.find(index_intersection_current)->second;
                ...
                if (original_duration > 0){
                    if (intersection_duration < original_duration) {
                        ...
                        cost_map.find(index_intersection_last)->second.at(index_intersection_current) = intersection_duration;
                        cost_map.find(index_intersection_current)->second.at(index_intersection_last) = intersection_duration;
                    }
                } else {
                    cost_current.at(index_intersection_last) = intersection_duration;
                    cost_map.find(index_intersection_last)->second.at(index_intersection_current) = intersection_duration;
                ...
                // connection_matrix[index_intersection_current][exit_current] = index_intersection_last;
                // connection_matrix[index_intersection_last][exit_last] = index_intersection_current;
                // if (cost_matrix[index_intersection_current][index_intersection_last] > 0){
                //     int original_duration = cost_matrix[index_intersection_current][index_intersection_last];
                //     int current_duration = intersection_duration;
                //     if (current_duration < original_duration) {
                //         cost_matrix[index_intersection_current][index_intersection_last] = current_duration;
                //         cost_matrix[index_intersection_last][index_intersection_current] = current_duration;
                //     }
                // } else {
                //     cost_matrix[index_intersection_current][index_intersection_last] = intersection_duration;
                //     cost_matrix[index_intersection_last][index_intersection_current] = intersection_duration;
                // }
                */
    ...
    // for(const auto& elem : cost_intersection){
    //     int id = elem.first;
    //     int south_exit =  std::get<0>(elem.second);
    //     int left_exit =  std::get<1>(elem.second);
    //     int north_exit =  std::get<2>(elem.second);
