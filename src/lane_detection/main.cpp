 ...
 * @return
 */
void LaneDetection::line_filtering(cv::Mat preprocessed_frame, cv::Vec4i &leftLane, cv::Vec4i &rightLane,
    ...
    */
    ...
    if (!rightLanes.empty()) {
        ...
        x2 /= rightLanes.size();
        y2 /= rightLanes.size();
        rightLane = cv::Vec4i(x2, y2, x1,
                              y1); // coordinates the way around, so that is matches the normal coordinate system
    }
...
bool LaneDetection::check_intersection(cv::Vec4i leftLane,
    ...
    for (const auto &horizontalLine: horizontalLines) {
        ...
        if (horizontalLine[0] < imageCenterX && horizontalLine[2] < imageCenterX) {
            ...
            float mLeft =
                    static_cast<float>(leftLane[1] - leftLane[3]) / (leftLane[0] - leftLane[2]); //m = (y2-y1)/(x2-x1)
...
LaneDetection::LaneDetection(LaneDetectionMode mode) {
    ...
    this->average_offset_array = std::array<double, 4>{0.0, 0.0, 0.0, 0.0};
